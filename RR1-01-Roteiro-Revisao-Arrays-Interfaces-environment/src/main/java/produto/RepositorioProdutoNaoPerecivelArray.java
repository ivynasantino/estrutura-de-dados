...
public class RepositorioProdutoNaoPerecivelArray {
	...
	public void remover(int codigo) {
		...
		ProdutoNaoPerecivel auxProdutos[] = new ProdutoNaoPerecivel[size];
		
		auxProdutos = produtos;
		
		for (int i = 0; i < auxProdutos.length; i++) {
			if (auxProdutos[i] != null) {
				if (auxProdutos[i].getCodigo() == codigo) {
					auxProdutos[i] = null;
				}
			}
		}
		
		ProdutoNaoPerecivel arrayProduto[] = new ProdutoNaoPerecivel[size];
		for (int i = 0; i < auxProdutos.length; i++) {
			if (auxProdutos[i] != null) {
				arrayProduto[i] = auxProdutos[i];
			}
		}
