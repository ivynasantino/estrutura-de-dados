...
public class RepositorioProdutoArrayList {
	...
	public void atualizar(Produto produto) {
		...
		Produto search = procurar(produto.getCodigo());
	...
	public void remover(int codigo) {
		ArrayList<Produto> auxProduto = new ArrayList<>();
		
		Produto search = procurar(codigo);
		
		if (search != null) {
			for (int i = 0; i < produtos.size(); i++) {
				if (produtos.get(i).getCodigo() != codigo) {
					Produto atualiza = auxProduto.get(i);
					atualiza = produtos.get(i);
				}
			}
